...
# Inspecting the schema of the resulting DataFrame
schema_info <- sdf_schema(test_results)
print(schema_info)

# Constructing the 'columns' parameter based on the schema

# Initialize vectors to store column names and types
column_names <- character()
column_types <- character()

# Loop through each column in schema_info
for (column in names(schema_info)) {
  column_names <- c(column_names, schema_info[[column]]$name)
  column_types <- c(column_types, schema_info[[column]]$type)
}

# Constructing the 'columns' parameter
columns_param <- setNames(object = column_types, nm = column_names)

# Running spark_apply with the 'columns' parameter
test_results_with_columns <- spark_df_partitioned %>% 
  spark_apply(mvar_fcast_fx, context = context_list, columns = columns_param, group_by = "unique_id", packages = FALSE)
